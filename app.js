...
function saveReplenishment() {
    ...
    const replenishment = {
        ...
        prateleiraAtivo: getActive('prateleiraAtivo'),
...
function recoverLastReplenishmentList() {
    ...
    try {
        ...
        const fields = [
            ...
            { active: 'prateleiraAtivo', value: 'prateleira' },
...
function navigateTo(screen) {
    ...
    if (screen === 'deliveries') {
        ...
        document.getElementById('faq-screen').classList.add('active');
    }
}
...
function initNavigation() {
    ...
    // Event delegation for per-item store selects and description inputs
    const itemsContainer = document.getElementById('order-items-container');
    if (itemsContainer) {
        itemsContainer.addEventListener('change', (e) => {
            const select = e.target.closest('.item-store-select');
            if (select) {
                const index = parseInt(select.dataset.index, 10);
                updateItemStore(index, select.value);
            }
        });
        itemsContainer.addEventListener('input', (e) => {
            const input = e.target.closest('.order-item-input');
            if (input) {
                const index = parseInt(input.dataset.index, 10);
                updateItemText(index, input.value);
            }
        });
    }
}
    ...
    if (lojaUnica) {
        ...
        container.innerHTML = orderItems.map((item, index) => `
            <div class="order-item">
                ...
                <input type="text" class="order-item-input" data-index="${index}" value="${escapeHtml(item.text)}" placeholder="Descrição">
        ...
        container.innerHTML = orderItems.map((item, index) => `
            <div class="order-item">
                ...
                <input type="text" class="order-item-input" data-index="${index}" value="${escapeHtml(item.text)}" placeholder="Descrição">
                <select class="item-store-select" data-index="${index}">
...
function toggleLojaUnica() {
    ...
    if (lojaUnica) {
        ...
        // Limpar a loja selecionada de todos os itens
        orderItems.forEach(item => { item.store = ''; });
    } else {
...
function generateDeliveryMessage() {
    ...
    if (description) {
        ...
        if (lojaUnica) {
            ...
            // Loja por item - mostrar a loja ao lado de cada item
            // Se nenhuma loja foi selecionada, usa DEPOSITO por padrão
            lines.forEach((line, index) => {
                if (line.trim()) {
                    const store = (orderItems[index] && orderItems[index].store) ? orderItems[index].store : 'DEPOSITO';
                    message += `${line.trim()} - ${store}\n`;
                }
            });
        }
    } else {
    ...
    if (lojaUnica) {
        message += `\n📍 Retirada: ${pickupLocation || 'DEPOSITO'}\n`;
    }
...
function generateSalesSummary() {
    ...
    if (totals.totalDelivery > 0) {
        ...
        if (totals.totalCartao > 0) {
            parts.push(`Crédito R$ ${totals.totalCartao.toFixed(2)}`);
            parts.push(`Débito R$ ${totals.totalCartao.toFixed(2)}`);
        }
        if (totals.totalPix > 0) {
            parts.push(`Pix R$ ${totals.totalPix.toFixed(2)}`);
        }
        if (parts.length > 0) {
            salesData = parts.join(' • ');
        }
    }
    ...
    let message;
    if (observation) {
        message = `Fechamento ${store}: ${observation} (${salesData})`;
    } else {
        message = `Fechamento ${store}: (${salesData})`;
    }
...
let message = '';
    ...
    if (delivery.retirada || !delivery.itensPedido || delivery.itensPedido.length === 0) {
        message += `\n📍 Retirada: ${delivery.retirada || 'DEPOSITO'}\n`;
    }
...
function generateReplenishmentMessage() {
    ...
    const categories = [
        ...
        { id: 'prateleira', icon: '📚', label: 'Prateleira' },
...
function toggleBTAddress() {
    ...
    function searchBTClient(query) {
        ...
        // Typing a search clears the new-client name field
        const clienteInput = document.getElementById('bt-cliente');
    ...
    function onBTNewClientInput(value) {
        // When typing a new client name, clear the search selection
        const searchInput = document.getElementById('bt-cliente-search');
        if (value && searchInput) {
            searchInput.value = '';
        }
        // Hide dropdown when starting a new client
        const dropdown = document.getElementById('bt-client-dropdown');
        if (dropdown) {
            dropdown.classList.add('hidden');
            dropdown.style.display = 'none';
        ...
        const saveClient = document.getElementById('bt-save-client');
        if (saveClient) {
            saveClient.checked = true;
        }
    }
...
// Build message
        ...
        if (taxidog) {
            ...
            if (includeMaps && endereco) {
                ...
                const mapsUrl = `https://www.google.com/maps/dir/?api=1&destination=${mapQuery}`;
                message += `📍 Localização: ${mapsUrl}\n`;
            }
        }
...
copyToClipboard(message);
    ...
    function saveBathgroomSchedule() {
        let cliente = document.getElementById('bt-cliente').value;
        const clienteSearch = document.getElementById('bt-cliente-search').value;
        const telefone = document.getElementById('bt-telefone').value;
        const petNome = document.getElementById('bt-pet-nome').value;
        const petRaca = document.getElementById('bt-pet-raca').value;
        const petPorte = document.getElementById('bt-pet-porte').value;
        ...
        if (!cliente && !clienteSearch) {
            showToast('Preencha o Cliente', 'error');
            return;
        }
        if (!petNome) {
            showToast('Preencha o Nome do Pet', 'error');
            return;
        }
        
        cliente = cliente || clienteSearch;
        ...
        const scheduleData = {
            ...
            createdAt: editingScheduleId 
                ? bathgroomSchedules.find(s => s.id === editingScheduleId)?.createdAt || new Date().toISOString()
                : new Date().toISOString()
        };
        
        if (editingScheduleId) {
            const index = bathgroomSchedules.findIndex(s => s.id === editingScheduleId);
            if (index !== -1) {
                bathgroomSchedules[index] = scheduleData;
            }
            editingScheduleId = null;
        } else {
            bathgroomSchedules.push(scheduleData);
        }
        localStorage.setItem('deliveryPets_bathgroom', JSON.stringify(bathgroomSchedules));
